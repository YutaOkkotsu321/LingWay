...
@Component({
  ...
  template: `
    <div class="page">
      ...
      <!-- Learned counter -->
      <div class="learned-counter">
        <span>Learned cards: <strong>{{ progress().mastered }} / {{ progress().total }}</strong></span>
      </div>
      ...
      } @else if (svc.flashcards().length === 0) {
        <div class="empty-state">
          <p class="empty-title">No flashcards yet</p>
          <p class="empty-sub">Create your first card to start practicing.</p>
          <a routerLink="/add-flashcard" class="empty-btn">+ Add Flashcard</a>
        </div>
      }
    </div>
  `,
  styles: [`
    ...
    .learned-counter {
      ...
      strong { color: var(--accent-green); font-family: var(--font-mono); font-size: 1.05rem; }
    }
    ...
    .empty-state {
      ...
      .empty-sub { color: var(--text-secondary); font-size: 0.9rem; }
      .empty-btn {
        margin-top: 10px;
